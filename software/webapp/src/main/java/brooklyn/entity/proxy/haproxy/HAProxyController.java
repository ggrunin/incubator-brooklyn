...
/**
 * An entity that represents an HAProxy proxy (e.g. for routing requests to servers in a cluster).
 * <p>
 ...
 */
@Catalog(name="HAProxy Server", description="A single HAProxy server. Provides HTTP and reverse proxy services", iconUrl="classpath:///nginx-logo.jpeg")
@ImplementedBy(HAProxyControllerImpl.class)
public interface HAProxyController extends AbstractController, HasShortName {

    MethodEffector<String> GET_CURRENT_CONFIGURATION =
            new MethodEffector<String>(HAProxyController.class, "getCurrentConfiguration");

    MethodEffector<Void> DEPLOY =
            new MethodEffector<Void>(HAProxyController.class, "deploy");
    ...
    ConfigKey<HAProxyConfigFileGenerator> SERVER_CONF_GENERATOR = ConfigKeys.newConfigKey(HAProxyConfigFileGenerator.class,
            "nginx.config.generator", "The server.conf generator class", new HAProxyDefaultConfigGenerator());

    @SetFromFlag("configTemplate")
    ConfigKey<String> SERVER_CONF_TEMPLATE_URL = HAProxyTemplateConfigGenerator.SERVER_CONF_TEMPLATE_URL;
    ...
    BasicAttributeSensorAndConfigKey<String> ACCESS_LOG_LOCATION = new BasicAttributeSensorAndConfigKey<String>(String.class,
            "nginx.log.access", "HAProxy access log file location", "logs/access.log");

    BasicAttributeSensorAndConfigKey<String> ERROR_LOG_LOCATION = new BasicAttributeSensorAndConfigKey<String>(String.class,
            "nginx.log.error", "HAProxy error log file location", "logs/error.log");
    ...
    public interface HAProxyControllerInternal {
