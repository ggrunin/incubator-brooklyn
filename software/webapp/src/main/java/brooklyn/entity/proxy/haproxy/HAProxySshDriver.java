...
/**
 * Start a {@link HAProxyController} in a {@link brooklyn.location.Location} accessible over ssh.
 */
public class HAProxySshDriver extends AbstractSoftwareProcessSshDriver implements HAProxyDriver {
    ...
    public static final Logger log = LoggerFactory.getLogger(HAProxySshDriver.class);
    public static final String NGINX_PID_FILE = "logs/nginx.pid";

    private boolean customizationCompleted = false;

    public HAProxySshDriver(HAProxyControllerImpl entity, SshMachineLocation machine) {
        super(entity, machine);

        entity.setAttribute(Attributes.LOG_FILE_LOCATION, getLogFileLocation());
        entity.setAttribute(HAProxyController.ACCESS_LOG_LOCATION, getAccessLogLocation());
        entity.setAttribute(HAProxyController.ERROR_LOG_LOCATION, getErrorLogLocation());
    }

    @Override
    public HAProxyControllerImpl getEntity() {
        return (HAProxyControllerImpl) super.getEntity();
    }
    ...
    public String getAccessLogLocation() {
        String accessLog = entity.getConfig(HAProxyController.ACCESS_LOG_LOCATION);
        return format("%s/%s", getRunDir(), accessLog);
    }

    public String getErrorLogLocation() {
        String errorLog = entity.getConfig(HAProxyController.ERROR_LOG_LOCATION);
    ...
    public void postLaunch() {
        entity.setAttribute(HAProxyController.PID_FILE, getRunDir() + "/" + AbstractSoftwareProcessSshDriver.PID_FILENAME);
    ...
    public void install() {
        ...
        boolean sticky = ((HAProxyController) entity).isSticky();
        ...
        String stickyModuleVersion = entity.getConfig(HAProxyController.STICKY_VERSION);
        ...
        if (isMac) {
            String pcreVersion = entity.getConfig(HAProxyController.PCRE_VERSION);
        ...
        String withLdOpt = entity.getConfig(HAProxyController.WITH_LD_OPT);
        if (isMac) withLdOpt = format("-L%s/pcre-dist/lib", getInstallDir()) + (Strings.isBlank(withLdOpt) ? "" : " " + withLdOpt);
        String withCcOpt = entity.getConfig(HAProxyController.WITH_CC_OPT);
    ...
    public void customize() {
        ...
        String archiveUrl = entity.getConfig(HAProxyController.STATIC_CONTENT_ARCHIVE_URL);
    ...
    private void reloadImpl() {
        ...
        //
        // Now we rely on HAProxyController always calling update (and thus reload) once it has started. This is
        ...
        Lifecycle lifecycle = entity.getAttribute(HAProxyController.SERVICE_STATE_ACTUAL);
